...
// Mostrar a seção de relatórios ao clicar no botão "Relatórios"
document.getElementById('menuRelatorios').addEventListener('click', function () {
    console.log('Botão de relatórios clicado'); // Log para depuração

    // Oculta todas as seções e mostra a seção de relatórios
    mostrarSeção('relatoriosSection');
    
    // Descomente esta linha se preferir gerar automaticamente
    // gerarRelatorio();
});

// Geração do relatório quando o botão "Gerar Relatório" é clicado
document.getElementById('gerarRelatorio').addEventListener('click', async function () {
    try {
        ...
        // Atualiza o conteúdo da página com os dados do relatório
        document.getElementById('resultadoRelatorio').innerHTML = `
            ...
            <p>Total de Garçons: ${relatorio.totalGarcons}</p>
            <p>Valor Total de Todos os Produtos Somados: R$ ${relatorio.valorTotalProdutos.toFixed(2).replace('.', ',')}</p>
        `;
    } catch (error) {
