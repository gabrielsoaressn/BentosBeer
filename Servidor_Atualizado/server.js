...
(async () => {
    try {
        ...
        const relatoriosManager = new Relatorios(db);
        
        app.get('/relatorio', async (req, res) => {
            try {
                const relatorio = await relatoriosManager.gerarRelatorio();
                res.json(relatorio);
            } catch (error) {
                console.error('Erro ao gerar relatório no servidor:', error);
