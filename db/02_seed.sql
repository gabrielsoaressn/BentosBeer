...
INSERT INTO produto (categoria_id, nome, descricao, preco) VALUES
  ...
  ((SELECT id FROM categoria WHERE nome='Chopes'), 'Chope Escuro 500ml', 'Malte torrado, corpo médio', 16.00),
  ...
  ((SELECT id FROM categoria WHERE nome='Cervejas'), 'Corona Long Neck',    '330ml com limão', 14.00),
  ...
  -- Drinks
  ((SELECT id FROM categoria WHERE nome='Drinks'), 'Caipirinha de Limão',  'Cachaça, limão taiti, açúcar', 18.00),
  ((SELECT id FROM categoria WHERE nome='Drinks'), 'Caipiroska de Morango','Vodca e morango fresco', 22.00),
  ((SELECT id FROM categoria WHERE nome='Drinks'), 'Gin Tônica',           'Gin, tônica, zimbro e limão siciliano', 26.00),
  ((SELECT id FROM categoria WHERE nome='Drinks'), 'Batida de Coco',       'Leite de coco e cachaça', 16.00),
  ((SELECT id FROM categoria WHERE nome='Drinks'), 'Aperol Spritz',        'Aperol, prosecco e água com gás', 28.00),
  ((SELECT id FROM categoria WHERE nome='Drinks'), 'Dose de Whisky',       'Duplo, com gelo', 22.00),
  -- Sem alcool
  ((SELECT id FROM categoria WHERE nome='Sem álcool'), 'Refrigerante Lata',   '350ml', 6.00),
  ((SELECT id FROM categoria WHERE nome='Sem álcool'), 'Água Mineral',        '500ml sem gás', 4.00),
  ((SELECT id FROM categoria WHERE nome='Sem álcool'), 'Água com Gás',        '500ml', 5.00),
  ((SELECT id FROM categoria WHERE nome='Sem álcool'), 'Suco Natural',        'Laranja, abacaxi ou maracujá', 9.00),
  ((SELECT id FROM categoria WHERE nome='Sem álcool'), 'Energético',          '250ml', 12.00),
  ((SELECT id FROM categoria WHERE nome='Sem álcool'), 'Café Expresso',       'Para fechar a noite', 6.00),
  -- Porcoes
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Porção de Calabresa',   'Com cebola e pão de alho', 38.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Isca de Frango',        'Empanada, com molho da casa', 42.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Batata Frita',          'Porção grande', 32.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Batata com Cheddar',    'Cheddar e bacon', 45.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Torresmo Mineiro',      'Crocante, com limão', 35.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Bolinho de Bacalhau',   '10 unidades', 44.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Tábua de Frios',        'Queijos, salame e azeitona', 58.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Pastel de Queijo',      'Unidade', 8.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Pastel de Carne',       'Unidade', 9.00),
  ((SELECT id FROM categoria WHERE nome='Porções'), 'Amendoim Torrado',      'Cortesia da casa em dobro', 12.00),
  -- Sobremesas
  ...
  ((SELECT id FROM categoria WHERE nome='Sobremesas'), 'Mousse de Maracujá',  'Taça', 12.00);
